...
class _DovizScreenState extends State<DovizScreen> {
  ...
  final TextEditingController _tutarController = TextEditingController(text: "1");
  ...
  Future<void> _verileriCek() async {
    try {
      ...
      } else if (response.containsKey('conversion_rates') && response['conversion_rates'] is Map) {
      ...
      asilKurlar.forEach((key, value) {
        ...
        if (k.contains('TIME') || k.contains('DATE') || k == 'SUCCESS' || k == 'BASE') return;
        ...
        if (value is num) {
          geciciMap[k] = {
            ...
            "oran": value.toDouble(),
            "degisim": 0.0, // Standart API'lerde değişim yüzdesi olmaz, 0 atıyoruz
          };
        } 
          ...
          geciciMap[k] = {
            ...
            "oran": double.tryParse(value['oran']?.toString() ?? value['alis']?.toString() ?? '0') ?? 0.0,
            "degisim": double.tryParse(value['degisim']?.toString() ?? '0') ?? 0.0,
          };
        }
      });

      // API 160 tane saçma kur gönderiyorsa sadece en önemlilerini filtrele (Opsiyonel ama şık durur)
      List<String> onemliKurlar = ['USD', 'EUR', 'GBP', 'TRY', 'XAU', 'CHF', 'CAD', 'JPY'];
  ...
  void _hesapla() {
    ...
    double tutar = double.tryParse(_tutarController.text.replaceAll(',', '.')) ?? 0.0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Kur Hesaplayıcı', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold, fontSize: 18)),
        centerTitle: false,
      ),
      body: _isLoading 
        ? const Center(child: CircularProgressIndicator(color: Color(0xFF0C4D3E)))
        : _hataMesaji.isNotEmpty
          ? Center(child: Text(_hataMesaji, style: const TextStyle(color: Colors.red)))
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text("Anlık Kurlar", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
                  const SizedBox(height: 16),
                  _anlikKurlarListesi(),
                  const SizedBox(height: 24),
                  _hesaplayiciKarti(),
                  const SizedBox(height: 24),
                  const Text("Hızlı Dönüşümler", style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.black87)),
  ...
  Widget _sonGuncellemeKarti() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: [BoxShadow(color: Colors.grey.withAlpha(10), blurRadius: 10, offset: const Offset(0, 4))]),
      child: Row(
        ...
        children: [
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text("Son Güncelleme", style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
              const SizedBox(height: 4),
              const Text("Anlık API Verisi", style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.black87)),
            ],
          ),
          Row(
            children: [
              ...
              const Text("Bağlantı Aktif", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.green)),
            ],
          )
        ],
      ),
  ...
  Widget _anlikKurlarListesi() {
    if (_kurlarMap.isEmpty) {
      return const Padding(padding: EdgeInsets.all(20), child: Text("Gösterilecek veri yok."));
    }
    ...
    return Container(
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.grey.withAlpha(10), blurRadius: 10, offset: const Offset(0, 4))]),
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(kod, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                      Text(_isimGetir(kod), style: TextStyle(fontSize: 11, color: Colors.grey.shade500)),
                    ],
                  ),
                ),
                Text("₺${gercekDeger.toStringAsFixed(2)}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                const SizedBox(width: 12),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                  decoration: BoxDecoration(color: isUp ? Colors.green.withAlpha(20) : Colors.red.withAlpha(20), borderRadius: BorderRadius.circular(8)),
                  child: Row(
                    children: [
                      Icon(isUp ? Icons.arrow_drop_up : Icons.arrow_drop_down, color: isUp ? Colors.green : Colors.red, size: 18),
                      Text("%${degisim.abs().toStringAsFixed(2)}", style: TextStyle(color: isUp ? Colors.green : Colors.red, fontWeight: FontWeight.bold, fontSize: 12)),
                    ],
                  ),
                ),
              ],
  ...
  Widget _hesaplayiciKarti() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: Colors.grey.withAlpha(10), blurRadius: 15, offset: const Offset(0, 8))]),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text("Kur Hesapla", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
              IconButton(icon: const Icon(Icons.sync, color: Colors.black87, size: 20), onPressed: _verileriCek),
            ],
          ),
          const SizedBox(height: 10),
          Container(
            ...
            child: Row(
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      Text(_bayrakGetir(_seciliGidenKur), style: const TextStyle(fontSize: 16)),
                      const SizedBox(width: 6),
                      Text(_seciliGidenKur, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                    ],
                  ),
                )
              ],
            ...
            child: Row(
              ...
              children: [
                Column(
                  ...
                  children: [
                    Text("Karşılık Gelen Tutar", style: TextStyle(fontSize: 11, color: Colors.grey.shade500)),
                    const SizedBox(height: 4),
                    Text("₺${_hesaplananTutar.toStringAsFixed(2)}", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black87)),
                  ],
                ),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                  ...
                  child: const Row(
                    children: [
                      ...
                      SizedBox(width: 6),
                      Text("TRY", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                    ],
                  ),
                )
              ],
          ...
          Container(
            ...
            child: Text("1 $_seciliGidenKur = ${_guncelOran.toStringAsFixed(2)} TRY", style: TextStyle(fontSize: 12, color: Colors.grey.shade600, fontWeight: FontWeight.w600)),
          ),
        ],
      ),
    );
  ...
  Widget _hizliDonusumler() {
    List<String> aktifKurlar = _kurlarMap.keys.where((k) => k != 'TRY').take(4).toList();
